...
from hashing import ListTable, LinearHashTable, ChainingHashTable, make_db_hash_table, process_camera_stream
#from hashing import simple_linear_plate_finder
...
TEST_RECORDS = test_records_sequence(110000)
    ...
    if not unique_hash:
        ...
        for record in TEST_RECORDS:
            ...
            if plate_hash not in hashes_so_far:
                ...
                yield record
            checked += 1
            if count >= max_records or checked >= n_test_records:
                raise StopIteration()
        return result
...
class TableTests(BaseTableTestMethods):
    ...
    def test_build_comparisons_unique(self):
        passed = False
        unique_hash = True
    ...
    def test_build_comparisons_non_unique(self):
        passed = False
        max_plates = int(self.n_test_slots * self.max_load_factor)
        for n_plates in range(1, max_plates, self.step):
    ...
    def test_actual_comps_add_unique(self):
        passed = False
        unique_hash = True
        max_plates = int(self.n_test_slots * self.max_load_factor)
        if max_plates > self.n_test_slots:
    ...
    def test_actual_comps_add_non_unique(self):
        passed = False
        max_plates = self.n_test_slots
        for n_plates in range(1, max_plates, self.step):
    ...
    def test_actual_comps_add_and_read(self):
        passed = False
        max_plates = self.n_test_slots
        for n_plates in range(1, max_plates, self.step):
            with self.subTest(n_plates=n_plates):
    ...
    def test_table_test_update(self):
        passed = False
        unique_hash = True
        max_plates = int(self.n_test_slots * self.max_load_factor)
        if max_plates > self.n_test_slots:
...
class BaseTestListTable(TableTests):
    ...
    def build_comps_upper_limit_non_unique(self, n_items, n_slots):
        """ Will be the same as forr the unique version as
        no hashing is done...
        """
        return (n_items * (n_items - 1)) / 2
...
class LargeTableTestsListTable(BaseTestListTable, LargeTableTests):
    pass
...
class BaseTestLinearProbing(BaseTableTestMethods):
    ...
    def build_comps_with_unique_hashes(self, n_items, n_slots=None):
        ...
        This will mean that zerp comparisons are used when adding
...
class TestMakingDbTable(BaseTableTestMethods):
    ...
    def test_1000_making_small_db_tables_from_file_data(self):
        ...
        for n_items, n_sighted, n_matches in settings:
            ...
            with self.subTest(n_items=n_items, n_sighted=n_sighted, n_matches=n_matches):
                passed = self.make_db_hash_table_test(
...
class TestMakingDbTableLinear(TestMakingDbTable):
...
class TestMakingDbTableChaining(TestMakingDbTable):
...
class SmallProcessCameraTests(BaseCameraStreamTests):
    ...
    def test_2000_small_process_camera_stream(self):
        ...
        for n_items, n_sighted, n_matches, db_size, results_size in settings:
            ...
            if not passed:
                ...
                #n_slots = n_items // 2
                #with self.subTest(n_items=n_items, n_sighted=n_sighted, n_matches=n_matches):
                #passed = self.make_db_hash_table_test(n_items, n_sighted, n_matches, n_slots)
                #if not passed:
                #break  # stop after the first subtest failure
...
class MediumProcessCameraTests(BaseCameraStreamTests):
    ...
    def test_2010_medium_process_camera_stream(self):
...
class LargeProcessCameraTests(BaseCameraStreamTests):
    ...
    def test_2020_large_process_camera_stream(self):
...
def all_tests_suite():
    ...
    # suite.addTest(unittest.makeSuite(LargeTableTestsListTable)) -> Test Case
    # --------------------------------------------------------------
